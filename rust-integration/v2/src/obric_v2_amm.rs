...
impl Amm for ObricV2Amm{
    ...
    fn quote(&self, quote_params: &QuoteParams) -> Result<Quote> {
        let (output_after_fee, protocol_fee, _) = if quote_params.input_mint.eq(&self.state.mint_x) {
             self.state.quote_x_to_y(quote_params.in_amount.clone(),self.current_x, self.current_y).unwrap()
        } else if quote_params.input_mint.eq(&self.state.mint_y) {
            self.state.quote_y_to_x(quote_params.in_amount.clone(),self.current_x, self.current_y).unwrap()
        } else {
            (0u64,0u64,0u64)
        };
        Ok(Quote {
    ...
    fn from_keyed_account(keyed_account: &KeyedAccount) -> Result<Self> where Self: Sized {

...
use jupiter_amm_interface::{Amm, KeyedAccount, Quote, QuoteParams, Swap, SwapAndAccountMetas, SwapParams};
use anyhow::{ Result};
...
impl Amm for ObricV3Amm {
    ...
    fn from_keyed_account(keyed_account: &KeyedAccount) -> Result<Self> {
        ...
        let (obligation,_) = Pubkey::find_program_address(&[consts::LARIX_OBLIGATION_SEED.as_bytes(),ss_trading_pair.mint_x.to_bytes().as_ref(),ss_trading_pair.mint_y.to_bytes().as_ref()], &PROGRAM_ID);
        Ok(Self {
    ...
    fn get_accounts_to_update(&self) -> Vec<Pubkey> {
        if self.larix_reserve_y.is_none() {
            return [self.key, self.state.x_price_feed_id, self.state.y_price_feed_id,
                consts::mint_to_larix_reserve(&self.state.mint_x).unwrap(),
                consts::mint_to_larix_reserve(&self.state.mint_y).unwrap()
            ].to_vec();
        } else {
            return [self.key, self.state.x_price_feed_id, self.state.y_price_feed_id].to_vec();
        }
    ...
    fn quote(&self, quote_params: &QuoteParams) -> Result<Quote> {
        let (output_after_fee, protocol_fee, _) = if quote_params.input_mint.eq(&self.state.mint_x) {
             self.state.quote_x_to_y(quote_params.in_amount.clone()).unwrap()
        } else if quote_params.input_mint.eq(&self.state.mint_y) {
            self.state.quote_y_to_x(quote_params.in_amount.clone()).unwrap()
        } else {
            (0u64,0u64,0u64)
        };
        Ok(Quote {
    ...
    fn get_swap_and_account_metas(&self, swap_params: &SwapParams) -> Result<SwapAndAccountMetas> {
        ...
        let mut account_metas = vec![
            ...
            AccountMeta::new( consts::mint_to_larix_reserve(&self.state.mint_x).unwrap(), false),
            AccountMeta::new( consts::mint_to_larix_reserve(&self.state.mint_y).unwrap(), false),
        ...
        if swap_params.source_mint.eq(&self.state.mint_y) {
            account_metas.push(AccountMeta::new_readonly(consts::larix::oracle::id(), false));
            account_metas.push(  AccountMeta::new_readonly(consts::mints::larix::id(), false));
            account_metas.push(AccountMeta::new(larix_reserve_x.liquidity.fee_receiver, false))
        }
